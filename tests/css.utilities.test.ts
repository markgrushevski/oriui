 ...
 * (`.ori-button.ori-variant_tonal`), never the older paired `base + modifier` pair. A leftover base
...
// (`.ori-variant_tonal`, `.ori-color_danger`), the block classes (`.ori-color-picker`) and the custom
...
describe('@oriui/css utility class model', () => {
    it('ships no bare paired-base class for the token axes (single-class utilities only)', () => {
        ...
        expect(
            ...
            `legacy paired-base rule found in: ${offenders.join(', ')} — the axes are single-class ` +
                `(.ori-variant_tonal), and a base in ori.utilities overrides the block's baked fill`
        ).toEqual([])
    })
    ...
    it('still ships every single-class variant utility the component blocks read', () => {
        ...
        for (const variant of ['fill', 'tonal', 'outline', 'text', 'plain']) {
