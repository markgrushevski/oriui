...
one value class (`ori-button ori-variant_soft`) — and the `.ori-color` axis already shipped without a
...
legacy paired form keeps rendering exactly as before: `ori-variant ori-variant_solid` already resolved
through `.ori-variant_solid` (same specificity, later in source order), so with the base gone the bare
