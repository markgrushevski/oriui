...
| `rovingIntent` · `resolveRovingIndex` · `ownsArrowKeys` · `textDirection` | Roving-tabindex toolkit — pure key→intent and intent→index math (`roving`), plus two DOM reads (`roving-dom`): whether a focused child owns the arrow keys, and the direction an element is laid out in. **Not a machine.** | [useToolbar](/headless/use-toolbar)                                                                               |
