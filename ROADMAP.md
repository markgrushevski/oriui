...
4. ✅ **Headless layer** — swappable behavior **contract** (`OriHeadless` plugin): **native** zero-dep
   ...
   in `@oriui/vue`. **Three framework adapters over the one framework-agnostic core**: `@oriui/headless/vue`
   (Vue `ComputedRef`s), `@oriui/headless/svelte` (Svelte stores) and `@oriui/headless/react` (plain values,
   re-rendered through `useSyncExternalStore`) — the **whole** composable surface at parity
   (`useDisclosure`/`useDialog`/`useCombobox`/`useMenu`/`useTabs`/`useToolbar`/`useToast`/`useColorPicker`/
   `useDismissable`/`useToken`/`useTheme`), proving the "behavior travels between frameworks" thesis.
