...
### ORI-I-84 — `<OriTabs>`'s fallback panel slot was cloned into every panel, duplicating `id`s into a hidden copy

`fixed` · severity `should-fix` · source: justpaint JP-I-04 (they filed it against themselves; the API question was ours), 2026-09-18

- **Where:** packages/vue/src/components/tabs/ori-tabs.vue — `v-for` over `tabs`, each panel rendering `<slot :name="panel-${tab.value}"><slot :tab="tab" /></slot>`
- **What:** the `#default` slot was a per-panel fallback, scoped with that panel's tab — documented, and right for a template that reads `{ tab }`. A template that ignored the scope was silently multiplied instead. Measured with two tabs and a login form in `#default`: **2 panels, 4 inputs, ids `email,password,email,password`, 2 distinct**. The sharp edge was not the extra DOM: `getElementById` (and therefore `<label for>`) resolves to the first copy in document order, which is the **hidden** panel whenever the active tab is not the first — measured `getElementById-lands-in-hidden-panel=true`. So the visible form's labels pointed at inputs nobody could reach, while `hidden` kept the duplicates out of the a11y tree, which is exactly why it stayed invisible until someone opened the inspector.
- **Fixed with option (b), the recorded recommendation: the fallback renders into the ACTIVE panel only.** One instance, no duplicate ids, no wasted mounts, and the scope it hands out is always the active tab. Every slot in the component now obeys one rule — one panel, one content — where the fallback alone used to be the exception that fanned out. Guarded by four tests: a shared template's ids stay unique, the single copy sits in a panel that is not `hidden`, the copy follows the selection, and the scope is the selected tab.
- **What it costs, and where that is written down:** uncontrolled DOM state inside a shared template (an unsent draft, a scroll position) does not survive a tab switch, because the content is created fresh in the panel you switch to. Content that must persist per tab belongs in `#panel-<value>`, which renders into its own panel regardless of selection. Stated in the component page's panel-slots section and in the slots table.
- **Also added, because it is the other half of the same silence:** a DEV warning when a `#panel-<value>` slot matches no tab (a typo, or a tab that was removed). Vue never warns about an unconsumed slot, so the panel silently fell back or rendered empty. The check is an exact match against the item values, so it cannot fire on correct code.
- **Decided against compound.** A compound Tabs would have made this defect structurally impossible, and that was the strongest argument for the shape. It was prototyped and measured on `poc/compound-tabs` (see `MEASUREMENTS-compound-tabs.md` there) and rejected on other grounds — the decision and its numbers are in DECISIONS.md.
...
`mitigated` · severity `should-fix` · source: reference audit 2026-09-19, measured here and by the skeptic

- **Where:** packages/vue/src/components/accordion/ori-accordion.vue — `<slot :item="item" />` inside `v-for`
- **What:** the same shape as ORI-I-84, worse in three ways. Measured: three items with a form in `#default` rendered ids `email,email,email`. There was **no per-value escape hatch** — the component had only `#title` and `#default`, so distinct content per section meant branching on `item.value` inside the shared template. Native `<details>` keeps every copy in the DOM whatever the open state. And in `multiple` mode two panels are open **simultaneously**: the skeptic measured that clicking the second open panel's `<label for>` focuses the FIRST panel's input, which Tabs' `hidden` at least kept invisible.
- **Mitigated by adding the escape hatch, not by gating the fallback — the status is `mitigated`, not `fixed`, because the fan-out itself is unchanged.** `#panel-<value>` now renders into its own section and no other, the same slot vocabulary and the same `panel-` prefix rationale as OriTabs; `#default` stays as the shared template for the sections that have no named slot. Tabs' remedy — render the fallback into the active panel only — has no analogue here, because `multiple` has no single active item, which is exactly what ORI-I-87 said a decision on Tabs would have to answer.
- **What remains open, stated plainly:** a `#default` template still renders once per item, so a caller who puts an `id` in it still gets one copy per section and, in `multiple` mode, still gets a `<label for>` resolving to the wrong one. The entry's original repro (three items, a form in `#default`, ids `email,email,email`) reproduces verbatim at HEAD, and `tests/accordion.test.ts` pins that on purpose. What changed is that there is now a correct path (`#panel-<value>`) and the docs point at it. Closing this for real means either deciding the fan-out is the documented contract and downgrading it, or finding a remedy that survives two open panels.
- **Deliberately NOT gated on `open`.** A closed `<details>` keeps its content in the DOM on purpose: that is what lets the browser find it with find-in-page and expand the section, and it is half the reason this component is native at all. Rendering only open sections would trade an id collision for a search regression.
- **Also added:** the same DEV warning as Tabs — a `#panel-<value>` slot that matches no item is named instead of silently falling back. Six tests cover the new slots: per-section rendering, unique ids in `multiple`, the fallback filling only the sections without a named slot, the `{ item }` scope, and the warning firing exactly on a mismatch.
...
- **Fixed** by measurement rather than by a guess. The variant is now `quiet` (`.ori-variant_quiet`) at `--ori-variant-opacity: 0.85`. The alpha was solved over all 96 role × skin × theme combinations from the composite the browser performs (`fg*a + bg*(1-a)` in sRGB, read off the `.ori-variant_text` probe, which is the same tone at opacity 1): 0.5 left **95 of 96** readings below AA, 0.75 left 23, **0.81 is the exact edge**, and 0.85 clears every reading with a worst of 4.95. (The count first published said 190 of 192: the sweep held two probes per combination — a button and a tag, which resolve to the same colour pair — so every population was doubled. Every ratio was right; only the denominator was not.) The analytic prediction was then confirmed against the real rasteriser — `e2e/text-contrast.spec.ts` reports 4.95 for `button-quiet` (sumi · light · success). The probe no longer carries `data-muted`, so the value is **asserted** from here on and cannot silently drift back.
